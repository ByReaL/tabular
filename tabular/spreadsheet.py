...
def strictaggregate(X, On, AggList, returnsort=False, keyfuncdict=None):
    ...
    argcounts = dict([(o,
                       f.__code__.co_argcount - (len(f.__defaults__) if
                                                 f.__defaults__ is not None else 0) if 'func_code' in dir(f) else 1)
...
def aggregate_in(Data, On=None, AggFuncDict=None, AggFunc=None, AggList=None,
    ...
    **See also:**
    ...
    U = np.array(utils.uniqify(ANLen))
    U.sort()
    ...
    **See also:**  :func:`tabular.spreadsheet.colstack`
    ...
    if isinstance(cols, list):
        if any([isinstance(x, np.ndarray) or isinstance(x, list) or
            ...
            assert names is not None and len(cols) == len(names), \
    ...
    if len(seq) > 1:
        ...
        if mode == 'nulls':
            names = utils.uniqify(utils.listunion([list(s.dtype.names)
                                                   for s in seq if s.dtype.names is not None]))
            formats = [max([s.dtype[att] for s in seq if s.dtype.names is not None
            ...
            return utils.fromarrays([utils.listunion([s[att].tolist()
                                                      if (s.dtype.names is not None and att in s.dtype.names)
    ...
    **See Also:**
    ...
    Result = colstack([(L[Names[0]][keycols])[0:0]] +
                      [deletecols(L[k][0:0], keycols)
                       for k in Names if deletecols(L[k][0:0], keycols) is not None])
        ...
        if len(Ref) > 0:
            ...
            NewRows = colstack([Ref0] +
                               [deletecols(L[j][D[i]], keycols).repeat(Reps0) if i > 0 else
                                ...
                                if deletecols(L[j][D[i]], keycols) is not None])
